...
export default function TelaCadastroAgenciaproduto(props) {
  ...
  async function consultarProdutosAgencia() {
    await fetch(urlAgencia + '/' + termo.codigo, { method: 'GET' })
      .then((resposta) => resposta.json())
      .then((retorno) => {
        if (retorno.status) setListaProdutosAgencia(retorno.listaProdutos);
        else alert(retorno.mensagem);
      })
      .catch((erro) => {
    ...
    return (
      <div>
        <Pagina>
          ...
          <TabelaAgenciaProduto setExibirTabela={setExibirTabela} listaProdutosAgencia={listaProdutosAgencia} alterarProdutoAgencia={alterarProdutoAgencia} excluirProdutoAgencia={excluirProdutoAgencia} />
        </Pagina>
      </div>
    );
  } else {
