...
export default function TelaCadastroUsuarioProduto(props) {
  ...
  async function consultarProdutosUsuario() {
    await fetch(urlUsuario + '/' + termo.codigo, { method: 'GET' })
      .then((resposta) => resposta.json())
      .then((retorno) => {
        if (retorno.status) setListaProdutosUsuario(retorno.listaProdutos);
        else alert(retorno.mensagem);
      })
      .catch((erro) => {
        alert('Erro: ' + erro.mensagem);
    ...
    return (
      <div>
        <Pagina>
          ...
          <TabelaUsuarioProduto setExibirTabela={setExibirTabela} listaProdutosUsuario={listaProdutosUsuario} alterarProdutoUsuario={alterarProdutoUsuario} excluirProdutoUsuario={excluirProdutoUsuario} />
        </Pagina>
      </div>
    );
  } else {
